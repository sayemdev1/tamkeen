  ...
  public function store(StoreProductRequest $request)
{
    ...
    $storeId = $user->store ? $user->store->id : Store::first()->id;
    ...
    if ($request->has('variants') && count($request->variants) > 0) {
        foreach ($request->variants as $index => $variant) {
            ...
            if ($index === 0) {
                ...
                $stockM= $stockM+ $variant['base_price'];

                // Handle cover_image and background_image
            ...
            if (isset($variant['background_image'])) {
                    $variant['background_image'] = $this->processBase64Image($variant['background_image'], 'product_images');
                }
           ...
           $childProduct= Product::create([
                ...
                if (isset($variant['images']) && !empty($variant['images'])) {
                    foreach ($variant['images'] as $imageBase64) {
                        // Process and save the image, returning its path
                        $imagePath = $this->processBase64Image($imageBase64, 'product_images');
    ...
    {
        if(str_starts_with($base64Image, 'storage')){
            return $base64Image;
        }
